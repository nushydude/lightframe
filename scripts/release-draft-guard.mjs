...
export async function runPreflightReleaseGuard({ apiGet, tag, expectedSha }) {
  const ref = await apiGet(`/git/ref/tags/${encodeURIComponent(tag)}`);
  const release = await findReleaseByTag({ apiGet, tag });
...
async function main() {
  ...
  assert.ok(
    ...
    'Usage: release-draft-guard.mjs <pre|post> <tag> <sha> [releaseId]'
  );
